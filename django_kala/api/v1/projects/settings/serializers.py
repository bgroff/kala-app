...
class ProjectPermissionSerializer(serializers.Serializer):
    ...
    def to_representation(self, obj):
        if type(obj) == ProjectPermission:
            return self.from_project_permission(obj)
        else:
            return self.from_record(obj)

    def from_project_permission(self, obj):
        representation = {
            'project': {
                'id': obj.id,
    ...
    def create(self, validated_data):
        ...
        if validated_data['permission'] == 'canInvite':
            return project.add_invite(user)
        if validated_data['permission'] == 'canCreate':
            return project.add_create(user)
        if validated_data['permission'] == 'canManage':
            return project.add_manage(user)
