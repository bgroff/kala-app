from api.v1.projects.settings.serializers import DocumentPermissionSerializer, ProjectPermissionSerializer
...
class ProjectPermissionsView(CreateAPIView, ListAPIView):
    ...
    def get_serializer_context(self):
        context = super(ProjectPermissionsView, self).get_serializer_context()
        context.update({
    ...
    def get_queryset(self):
        ...
        with connection.connection.cursor(cursor_factory=NamedTupleCursor) as cursor:
            cursor.execute("""
                    SELECT projects_projectpermission.id,
                    ...
                    FROM   kala_user
                           LEFT JOIN projects_projectpermission
                                  ON projects_projectpermission.user_id = kala_user.id AND projects_projectpermission.project_id = %s
                    ...
                    """, [self.project.id, self.project.id, self.project.organization.id, tuple(self.available_users)])
            return cursor.fetchall()


class ProjectPermissionView(UpdateAPIView, DestroyAPIView):
    serializer_class = ProjectPermissionSerializer
    permission_classes = []
    queryset = ProjectPermission.objects.all()

    def dispatch(self, request, project_pk, pk, *args, **kwargs):
        try:
            self.project = get_object_or_404(Project.objects.select_related(), pk=project_pk)
            self.permission = get_object_or_404(ProjectPermission, pk=pk)

            if not self.project.can_manage(request.user):
                raise PermissionDenied('You do not have permission to modify this project permission')

            if self.permission.project.pk != self.project.pk:
    ...
    def get_serializer_context(self):
        context = super(ProjectPermissionView, self).get_serializer_context()
        context.update({
            'user': self.request.user,
            'project': self.project
        })
            ...
            cursor.execute("""
                    ...
                    FROM   kala_user
                           LEFT JOIN documents_documentpermission
                                  ON documents_documentpermission.user_id = kala_user.id AND documents_documentpermission.document_id = %s
                           ...
                           LEFT JOIN organizations_organizationpermission
                                  ON organizations_organizationpermission.organization_id = kala_projects.organization_id
                                     AND organizations_organizationpermission.user_id = kala_user.id
                    WHERE kala_user.id in %s ORDER BY last_name;
                    """, [self.document.id, self.document.id, self.document.project.id, self.document.project.organization.id, tuple(self.available_users)])
            return cursor.fetchall()
